...
## 2.1 Lectura geométrica: columnas unánimes y columnas divididas

La condición \(n\equiv0\pmod4\) tiene una explicación geométrica directa que conviene entender antes de continuar.

Dada una tríada \((A,B,C)\) con los tres conjuntos equilibrados y mutuamente ortogonales, cada posición \(i\in[n]\) produce una columna de tres signos \((H_A(i), H_B(i), H_C(i))\). Hay dos tipos de columna:

- **Columna unánime:** los tres signos son iguales (\(+++\) o \(---\)). No contribuye a ninguna distancia Hamming entre pares.
- **Columna dividida:** dos signos son iguales y uno distinto. Contribuye exactamente 2 al total de las tres distancias Hamming \(d(A,B)+d(A,C)+d(B,C)\).

La condición de ortogonalidad exige \(d(H_A,H_B)=d(H_A,H_C)=d(H_B,H_C)=n/2\), luego la suma total es \(3n/2\). Como cada columna dividida aporta exactamente 2, el número de columnas divididas es \(k=3n/4\). Para que \(k\) sea entero necesitamos \(n\equiv0\pmod4\).

Además, el número de columnas unánimes es \(n-k=n/4=m\). En el caso \(n=4\): exactamente \(k=3\) columnas divididas y \(1\) columna unánime.

Esa única columna unánime es la que, en la caracterización Hadamard de la sección 7.1, corresponde a la fila constante \(u=(1,1,1,1)\). El equilibrio de cada bloque (\(|A|=n/2\)) fuerza que cada columna dividida tenga exactamente dos \(+1\) y un \(-1\), o dos \(-1\) y un \(+1\) — lo que distribuye los signos uniformemente y completa la estructura de Hadamard.
...
Por tanto, el invariante \(t\) es **completo**: dos tríadas tienen el mismo \(t\) si y solo si son equivalentes bajo \(S_n\).

---

## Teorema

Las tríadas equilibradas y mutuamente ortogonales de dimensión \(n=4m\) se descomponen bajo \(S_n\) como

\[
\mathcal T_n/S_n = \{0,1,\ldots,m\},
\]

es decir, en exactamente \(m+1\) órbitas \(\mathcal O_0,\mathcal O_1,\ldots,\mathcal O_m\). La órbita de parámetro \(t\) tiene tamaño
...
**Separación.** El valor \(t=|A\cap B\cap C|\) es invariante bajo permutaciones de coordenadas, por lo que tríadas en órbitas distintas tienen \(t\) distinto.

**Completitud.** Si dos tríadas \((A,B,C)\) y \((A',B',C')\) tienen el mismo \(t\), sus 8 átomos de Venn tienen exactamente los mismos tamaños \((t,m-t,m-t,m-t,t,t,t,m-t)\).

**Lema** (transitividad sobre particiones equipotentes). *Sea \([n] = B_1 \sqcup \cdots \sqcup B_8\) una partición con \(|B_i|=a_i\), y sea \([n] = B_1' \sqcup \cdots \sqcup B_8'\) otra con \(|B_i'|=a_i\) para todo \(i\). Entonces existe \(\sigma\in S_n\) con \(\sigma(B_i)=B_i'\) para todo \(i\).*

*Demostración.* Para cada \(i\), elijamos una biyección \(\phi_i:B_i\to B_i'\) (posible porque \(|B_i|=|B_i'|\)). Definimos \(\sigma(x)=\phi_i(x)\) para el único \(i\) con \(x\in B_i\). Como los bloques son disjuntos y cada \(\phi_i\) es biyectiva, \(\sigma\) es una permutación de \([n]\) con \(\sigma(B_i)=B_i'\). \(\square\)

Aplicando el lema a los ocho átomos de Venn de las dos tríadas, obtenemos \(\sigma\in S_n\) con \(\sigma(A)=A'\), \(\sigma(B)=B'\), \(\sigma(C)=C'\). Luego son \(S_n\)-equivalentes.

*Verificación constructiva independiente.* Para \(n=8\), se construyó \(\sigma\) explícita para las 45360 tríadas (exhaustivo). Para \(n=12\), se verificó en 2000 pares aleatorios con mismo \(t\).

**Tamaño.** El estabilizador de cualquier tríada es \(\operatorname{Stab}(\mathcal O_t)\cong(S_t)^4\times(S_{m-t})^4\), de orden \((t!)^4((m-t)!)^4\). Por el teorema órbita-estabilizador:
...
Se realizó una búsqueda exhaustiva en arXiv, Springer y Google Scholar. La cadena específica
...
**no se ha encontrado en la literatura**. Los trabajos más cercanos son:

- Matolcsi et al. (2026): tríadas de bases mutuamente no sesgadas en \(\mathbb{C}^d\) — contexto complejo, sin el invariante \(t\) ni la fórmula de órbitas.
- Mesner y Bhattacharya (1990): esquemas de asociación sobre tríadas — clasificación por grupos 2-transitivos, no por vectores equilibrados ortogonales.
- Balmaceda y Briones (2025): familias de esquemas de asociación sobre tríadas — antecedente técnico más próximo, sin superposición directa.

Ninguno de ellos contiene la fórmula \(|\mathcal{O}_t|=n!/((t!)^4((m-t)!)^4)\) ni la identificación \(P_{48}\cong B_3\).
...
- Matolcsi, M., Matszangosz, Á. K., Varga, D., Weiner, M. *Triplets of mutually unbiased bases*. Journal of Algebraic Combinatorics 63, 26 (2026). DOI:10.1007/s10801-026-01506-x
- Mesner, D., Bhattacharya, P. *Association schemes on triples*. Journal of Combinatorial Theory, Series A 55 (1990), 255–272.
- Balmaceda, J., Briones, D. *Families of association schemes on triples from two-transitive groups*. Ars Mathematica Contemporanea 25(3) (2025). arXiv:2107.07753.
