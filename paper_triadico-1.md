# Bases triádicas ortogonales en \(\{-1,+1\}^n\): órbitas, el caso excepcional \(n=4\) y una fórmula general
...
**Teorema 1** (Descomposición en órbitas). *Sea \(m = n/4\). Las tríadas perfectas de dimensión \(n\) se descomponen bajo la acción de \(S_n\) sobre las coordenadas en exactamente \(m+1\) órbitas, clasificadas por el invariante*

\[
t = |A \cap B \cap C| \in \{0, 1, \ldots, m\}.
\]

*La órbita \(\mathcal{O}_t\) consiste en todas las tríadas ordenadas con triple intersección de tamaño \(t\), y tiene tamaño*
...
Dos tríadas pertenecen a la misma \(S_n\)-órbita si y solo si sus secuencias de átomos de Venn son idénticas (salvo permutaciones dentro de cada átomo), lo que ocurre exactamente cuando comparten el mismo \(t\). El estabilizador de cualquier tríada con parámetro \(t\) es \((S_t)^4 \times (S_{m-t})^4\), de orden \((t!)^4 ((m-t)!)^4\). El teorema órbita-estabilizador da la fórmula. \(\square\)
...
| \(n\) | \(m\) | Orbits | \(t\) | \(|\mathcal{O}_t|\) ordered | Verificado |
|---:|---:|---:|---:|---:|---|
...
|---:|---:|---:|---:|---|
| 4 | 6 | 48 | 2 | No (\(m\) impar) |
| 8 | 70 | 45360 | 3 | \(\mathcal{O}_1\), size \(8!\) |
| 12 | 924 | 60614400 | 4 | No (\(m\) impar) |
| 16 | 12870 | 114144030000 | 5 | \(\mathcal{O}_2\), size \(16!/(2!)^8\) |
...
6. \(C(LA, LB) = C(A,B)\) for all \(L \in B_3\)
...
\text{8 orthogonal bases} \;\longrightarrow\;
P_{48} \cong B_3 \;\longrightarrow\;
\mathrm{Cay}(B_3,S) \cong Q_3 \,\square\, K_{3,3}.
\]

**Lo que es nuevo.** Los objetos individuales — matrices de Hadamard de orden 4, \(J(4,2)\), el sistema de raíces \(A_3\), el producto cartesiano \(Q_3 \,\square\, K_{3,3}\), el análisis Walsh-Hadamard — son clásicos. La contribución es:

1. La fórmula de órbitas del Teorema 1, que clasifica las tríadas perfectas en todas las dimensiones válidas.
2. La identificación de \(n=4\) como el caso excepcional en que la fórmula de órbitas produce un grupo (\(B_3\)) en lugar de simplemente un torsor (\(n=8\)) o un espacio homogéneo mayor.
3. La verificación de que la cascada se extiende: para \(n=8\), la órbita dominante es un \(S_8\)-torsor; para \(n=12\), las cuatro órbitas coinciden exactamente con la fórmula.
